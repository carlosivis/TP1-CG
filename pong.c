...
void drawbackground(){
	...
	switch (backgroundstate){ //Selects proper background texture depending on the gamemode
		...
		case Pause_Texture :  glBindTexture(GL_TEXTURE_2D,idPause);break;
			}
	glBegin(GL_POLYGON);
		glTexCoord2f(0,0);
		glVertex2f(-350,-500);
...
void gameloop(int valor){
	...
	if(keyboard[112]){
		...
		keyboard[112] = 0;//Setting ESC to 0 here ensures that the ESC key toggles the pause menu
	}	

	if ((gamestate==Pause)&&(keyboard[27]))//Exits game when the game is paused and the x key is pressed
		exit(0);
	...
	switch (gamestate){
		...
		if (keyboard[27])//Key to exit from the menu.
			exit(0);
		} break;
...
void drawscene(){
	...
	switch (gamestate){//Detects the game mode and draws accordingly.
		...
		case SplashScreen : drawbackground();break;
...
int main(int argc, char** argv) {
   ...
   glutCreateWindow("Pong Animal Simulator");
